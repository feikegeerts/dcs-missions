...
const expectedCatalogue: NewValuationCatalogueRow = {
  ...
  effectiveDate: ordnanceCatalogueV2.effective_date,
  currency: ordnanceCatalogueV2.currency,
  pricingConvention: ordnanceCatalogueV2.pricing_convention,
  typeKeySource: ordnanceCatalogueV2.type_key_source,
};

const expectedItems: readonly NewValuationItemRow[] =
  ordnanceCatalogueV2.items.map((item) => ({
    catalogue: ordnanceCatalogueV2.catalogue,
...
async function seed(): Promise<void> {
  ...
  const violations = validateCatalogue(ordnanceCatalogueV2);
