...
test("ACM keeps Survival lifecycle settings and uses close-range spawning", function()
  local acm = dofile("src/missions/duel-dynamic-acm/config.lua")
  local survival = dofile("src/missions/air-superiority-survival/config.lua")
  assert(acm.gameplay.spawn_distance_min_sm == 20)
  assert(acm.gameplay.spawn_distance_max_sm == 20)
  assert(acm.gameplay.lives_per_player == survival.gameplay.lives_per_player)
  assert(acm.gameplay.respawn_delay_s == survival.gameplay.respawn_delay_s)
  assert(acm.gameplay.wave_escalation_every == nil)
  assert(acm.gameplay.wave_tier_every == nil)
  assert(acm.gameplay.max_package_size == nil)
  assert(acm.gameplay.bandit_task == nil)
  assert(acm.gameplay.wave_donor_tiers == nil)
end)
