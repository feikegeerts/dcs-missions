...
succeeds("bridge acknowledgement retry is idempotent only when already covered", function()
  local runtime = new_runtime()
  check(runtime:begin("producer-ack-retry", "run-ack-retry"))
  check(runtime:ack(1))
  local retried, marker = runtime:ack(1)
  check(retried)
  equal(marker, "already-applied")
  fails(function()
